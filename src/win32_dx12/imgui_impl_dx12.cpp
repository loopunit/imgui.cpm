...
static D3D12MA::Allocation*		   g_pFontTextureResourceAlloc = NULL;
...
struct ImGui_ImplDX12_RenderBuffers
{
	...
	D3D12MA::Allocation* IndexBufferAlloc;
	D3D12MA::Allocation* VertexBufferAlloc;
	int					 IndexBufferSize;
...
struct ImGuiViewportDataDx12
{
	...
	ImGuiViewportDataDx12()
	{
		...
		for (UINT i = 0; i < g_numFramesInFlight; ++i)
		{
			...
			FrameRenderBuffers[i].IndexBufferAlloc	= NULL;
			FrameRenderBuffers[i].VertexBufferAlloc = NULL;
	...
	~ImGuiViewportDataDx12()
	{
		...
		for (UINT i = 0; i < g_numFramesInFlight; ++i)
		{
			...
			IM_ASSERT(FrameRenderBuffers[i].IndexBufferAlloc == NULL && FrameRenderBuffers[i].VertexBufferAlloc == NULL);
		}
...
static void ImGui_ImplDX12_DestroyRenderBuffers(ImGui_ImplDX12_RenderBuffers* render_buffers)
{
	...
	SafeRelease(render_buffers->IndexBufferAlloc);
	SafeRelease(render_buffers->VertexBufferAlloc);
...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx)
{
	...
	if (fr->VertexBuffer == NULL || fr->VertexBufferSize < draw_data->TotalVtxCount)
	{
		...
		SafeRelease(fr->VertexBufferAlloc);
		...
		if (g_allocator->CreateResource(&allocationDesc, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, &fr->VertexBufferAlloc, IID_PPV_ARGS(&fr->VertexBuffer)) < 0)
			return;
	}
	if (fr->IndexBuffer == NULL || fr->IndexBufferSize < draw_data->TotalIdxCount)
	{
		SafeRelease(fr->IndexBuffer);
		SafeRelease(fr->IndexBufferAlloc);
		...
		if (g_allocator->CreateResource(&allocationDesc, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, &fr->IndexBufferAlloc, IID_PPV_ARGS(&fr->IndexBuffer)) < 0)
			return;
	}
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
	...
	// Upload texture to graphics system
	{
		...
		D3D12MA::Allocation* pTextureAlloc = NULL;
		g_allocator->CreateResource(&allocationDesc, &desc, D3D12_RESOURCE_STATE_COPY_DEST, NULL, &pTextureAlloc, IID_PPV_ARGS(&pTexture));
		...
		D3D12MA::Allocation* uploadBufferAlloc = NULL;
		HRESULT				 hr = g_allocator->CreateResource(&allocationDesc, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, &uploadBufferAlloc, IID_PPV_ARGS(&uploadBuffer));
		...
		SafeRelease(g_pFontTextureResourceAlloc);
		g_pFontTextureResource		= pTexture;
		g_pFontTextureResourceAlloc = pTextureAlloc;
	}
